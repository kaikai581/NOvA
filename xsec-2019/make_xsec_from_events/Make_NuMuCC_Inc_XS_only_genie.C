...
void Make_NuMuCC_Inc_XS_only_genie(const char* fOutname, int sidx = 0, int eidx = -1, bool do_data = false){
  ...
  TFile* fdata = new TFile(Form("%s/data.root" ,dirIn.c_str()),"read");
  TFile* fmc;
  //flux:
  ...
  if(do_data){
    dir_data = fdata->GetDirectory("Data");
    fake_xs = "data";
  }
  std::string namesyst = "";
  TDirectory* dir_mc;
