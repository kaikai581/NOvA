...
syst_names = ['fakedata', 'nominal']

src_dir = '/pnfs/nova/scratch/users/slin/cafana_on_grid_output/dropbox'

outfns = ['tunfold_'+syst_name+'.root' for syst_name in syst_names]
...
matchstr = os.path.join(src_dir, 'tunfold_study_'+systname+'*.txt')

# collect all files and loop through
for infn in glob.glob(matchstr):
    tr.ReadFile(infn, 'muang_reco/F:muke_reco/F:nue_reco/F:hadeavail_reco/F:nue_osc_reco/F:hade_reco/F:muang_true/F:muke_true/F:nue_true/F:hadeavail_true/F:hade_true/F:xsec_weight/F:ppfx_weight/F:muke_cut/I:muang_cut/I:phasespace_cut/I:truesig/I', ' ')
outf.Write()